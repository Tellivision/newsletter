-- Newsletter Pro Database Setup Script (Final Version)
...
-- Create updated_at triggers (only if they don't already exist)
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_subscriber_lists_updated_at') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_list_memberships_updated_at') THEN
...
DROP POLICY IF EXISTS "Enable insert access for all users" ON public.subscribers;
DROP POLICY IF EXISTS "Enable update access for all users" ON public.subscribers;
DROP POLICY IF EXISTS "Enable delete access for all users" ON public.subscribers;
DROP POLICY IF EXISTS "Users can manage their own newsletters" ON public.newsletters;
DROP POLICY IF EXISTS "Enable read access for templates" ON public.templates;
DROP POLICY IF EXISTS "Users can manage their own subscriber lists" ON public.subscriber_lists;
DROP POLICY IF EXISTS "Enable access for list memberships" ON public.list_memberships;
DROP POLICY IF EXISTS "Enable access for newsletter recipients" ON public.newsletter_recipients;
